...
"""
    ...
    开发环境打包：
    cd /home/sendoh/
    tar -zcvf qdata-cloud.env.tar.gz qdata-cloud/

    #传输到对端
    scp qdata-cloud.env.tar.gz root@10.10.160.69:/home/sendoh
    tar zxvf qdata-cloud.env.tar.gz -C /home/sendoh/ && echo "/home/sendoh/qdata-cloud/packages/mysql/lib" > /etc/ld.so.conf.d/mysql-qdata.conf && ldconfig && source /home/sendoh/qdata-cloud/venvs/ci/bin/activate && python -c "import time;import datetime;import cx_Oracle; import MySQLdb"
    ＃安装新包，请在active之行后，用pip安装
...
class QTableCat():
    """解析table配置文件，生成table,vtable,twins信息，并保存到数据库"""
    def __init__(self,v_meta_hooker = None,v_crusher_db = None,v_job_name = "",v_job_ctype = "new"):
        ...
        self.cgv_job_name = v_job_name;
        self.cgv_job_id = 0;
        #对于vtable的处理模式，new：新建，append：追加，recreate：重建
        self.cgv_job_ctype = v_job_ctype;
        self.cgv_vtable_ctype = self.cgv_job_ctype;
    ...
    def cgf_job_meta_gen(self):
        """生成jobmeta信息"""
        if (len(self.cgv_job_name) <= 0 or self.cgv_job_name is None):
            print "QTableCat.cgf_job_meta_gen : Job name should not be Null!!!";
            os._exit(0);
        self.cgv_job_id = self.cgv_meta_hooker.cgf_create_job_info(v_job_name = self.cgv_job_name);
            
    def cgf_table_meta_gen(self):
        """1.vtable由多个或一个table组成，vtable一种是新建new，一种是append加子表，一种是recreate重建
           2.一个vtable下的table不允许重复，程序会自动忽略重复table
           3.new vtable，如果已经存在，会忽略该vtable，append vtable，如果存在才扩展，recreate直接重建，没有有新增
           4.针对每个子table，会生成一个task
           5.recreate会重置vtable的single table
        """
        try:
            ...
            for i in xrange(len(self.cgv_virtual_tabname_scope)):
                ...
                # 生成vtable元信息，函数的返回值决定后续逻辑，-1表示新建vtable已经存在，其他值表示继续进行
                v_vtable_id,v_stab_exname = self.cgv_meta_hooker.cgf_create_vtable_info(v_vtab_name = v_tab_vname,v_stab_name = v_tab_sname,
                    v_vtab_status = v_vtab_status,v_source_target = v_source_target,v_vtable_ctype = self.cgv_vtable_ctype);

                if (v_vtable_id == -1):
                    # new模式，且已经存在同名vtable，不做处理
                    print "Virtual table " + v_tab_vname + "(" + v_tab_sname + ")" + " already existed,Please change other name!!!";
                    print "You should init this vtable lonely once again!!!";
                    # 进入下个vtable的处理
                    continue;
                elif (v_vtable_id == -2):
                    # append模式，但是vtable不存在，不做处理
                    print "Virtual table " + v_tab_vname + "(" + v_tab_sname + ")" + " dosen't existed,append canceled!!!";
                    continue;
                else:
                    if (self.cgv_vtable_ctype == "new"):
                        # 新建对应的single table
                        self.cgf_single_table_ddl(v_tab_name = v_tab_sname,v_is_truncate = False);
                    elif (self.cgv_vtable_ctype == "recreate"):
                        # 清理掉现有的子表和task
                        self.cgv_meta_hooker.cgf_reset_vtable_info(v_vtable_id = v_vtable_id);

                        # recreate需要truncate single table
                        if (v_stab_exname is not None and len(v_stab_exname) > 0):
                            # 存在，就truncate,注意是返回的v_stab_exname
                            self.cgf_single_table_ddl(v_tab_name = v_stab_exname,v_is_truncate = True);
                        else:
                            # 不存在，就新建，注意此时是根据v_tab_sname新建
                            self.cgf_single_table_ddl(v_tab_name = v_tab_sname,v_is_truncate = False);
                    elif (self.cgv_vtable_ctype == "append"):
                        # 不需要做single table的处理
                        pass;
                    ...
                    for j in xrange(v_start_pos,v_end_pos + 1):
                        ...
                        if (v_tab_descstring_list.get(v_tab_descstring) is None):
                            ...
                            self.cgv_meta_hooker.cgf_create_task_info(v_task_name = v_task_name,v_parent_id = v_table_id,
                                v_job_id = self.cgv_job_id,v_task_type = "pull");
                        else:
                            pass;
        except Exception as e:
    ...
    def cgf_twins_meta_gen(self):
        """生成校验对的meta信息
           按照源端vtable，目标端vtable进行配对，并生成task
           已经存在，不做任何处理
        """
        try:
            for i in xrange(len(self.cgv_twins_data_list)):
                ...
                # 为该twins创建一个task，便于后续调度
                self.cgv_meta_hooker.cgf_create_task_info(v_task_name = v_task_name,v_parent_id = v_twins_id,
                    v_job_id = self.cgv_job_id,v_task_type = "check");

        except Exception as e:
            print "QTableCat.cgf_twins_meta_gen get error:" + str(e).replace("'","");
            os._exit(0);

    def cgf_single_table_ddl(self,v_tab_name = None,v_vtable_id = 0,v_is_truncate = False):
        """创建对比表，表名按照QT0000000001模式,仅支持Oracle做为对比库,未来可以引入PostgreSQL"""
        try:
            v_ddl_sql = "";
            if (self.cgv_crusher_db.cgv_dbtype == "Oracle"):
                if (v_tab_name is not None and len(v_tab_name) > 0):
                    # 使用表名为参数，直接拼好ddl sql
                    ...
                    # 使用vtable_id参数，需要先找出stable name，再拼ddl sql
                    v_stable_name = self.cgv_meta_hooker.cgf_get_vtable_stable(v_vtable_id = v_vtable_id);
                    if (v_is_truncate == False):
                        # 创建表
                        v_ddl_sql = "create table " + v_stable_name + \
                        "(keystr varchar2(2048),hashvalue varchar2(512),special_key varchar2(256)," + \
                        "tabsharding_id number,datastr clob) pctfree 0 nologging";
                    else:
                        # 复用表，直接truncate
                        v_ddl_sql = "Truncate Table " + v_stable_name;
            ...
            print "QTableCat.cgf_single_table_ddl get error :" + str(e).replace("'","");
...
class QMetaHooker():
    """负责元数据维护管理，目前采用MySQL库作为元数据管理,update函数由外部做事务与错误管理，便于数据一致性"""
    ...
    def cgf_commit_trans(self):
        """提交事务"""
        self.cgv_conn_db.commit();

    def cgf_rollback_trans(self):
        """回滚事务"""
    ...
    def cgf_get_unique_key(self):
        ...
        try:
            ...
            print "QMetaHooker.cgf_get_unique_key get error:" + str(e).replace("'","");
            self.cgv_conn_db.rollback();
            self.cgv_meta_db.cgf_disconnect_db();
            os._exit(0);

    def cgf_create_job_info(self,v_job_name = "",v_job_ctype = "new"):
        """job创建函数，返回taskid，失败的话程序直接退出"""
        try:
            v_job_id = 0;
            if (len(v_job_name) < 1):
                print "Job name is Null,exit!!!!"
                os._exit(0);

            if (self.cgv_db_cur):
                # 获取job信息
                v_sql = "select id from q_job_list where job_name = '%s'" % (v_job_name);
                self.cgv_db_cur.execute(v_sql);
                v_rows_list = self.cgv_db_cur.fetchall();
                if (len(v_rows_list) > 0):
                    # 已经存在，根据ctype进行
                    if (v_job_ctype == "new"):
                        print "Job " + v_job_name + " already existed,please change a different jobname to create!!!";
                        os._exit(0);
                    elif (v_job_ctype in ("append","recreate")):
                        return v_rows_list[0][0];
                else:
                    # 不存在该Job，新建，并返回jobid
                    v_sql = "insert into q_job_list(job_name,job_status,begin_time) " + \
                    "values('%s','preparing',CURRENT_TIMESTAMP)" % (v_job_name);

                    self.cgv_db_cur.execute(v_sql);
                    # 获取插入的主键id，必须在事务commit之前，否则为0
                    v_job_id = self.cgv_conn_db.insert_id();
                    self.cgv_conn_db.commit();

                    return v_job_id;
        except Exception, e:
            print "QMetaHooker.cgf_create_job_info get error:" + str(e).replace("'","");
            self.cgv_conn_db.rollback();
            self.cgv_meta_db.cgf_disconnect_db();
            os._exit(0);

    def cgf_update_job_status(self,v_job_id = None,v_job_name = "",v_job_status = None,v_error_info = ""):
        """设置job的状态，preparing准备，gooooing进行中，finished全部完成,error表示出错"""
        if (self.cgv_db_cur):
            v_sql = """update q_job_list set job_status = '%s',
                    gmt_modify = CURRENT_TIMESTAMP,
                    error_info = '%s'
                    where id = %s or job_name = '%s'""" % (v_job_status,v_error_info,v_job_id,v_job_name);
            self.cgv_db_cur.execute(v_sql);


    def cgf_update_job_btime(self,v_job_id = None,v_job_name = ""):
        """设置job的开始时间"""
        if (self.cgv_db_cur):
            v_sql = """update q_job_list set 
                    gmt_modify = CURRENT_TIMESTAMP,
                    begin_time = CURRENT_TIMESTAMP
                    where id = %s or job_name = '%s'""" % (v_job_id,v_job_name);
            self.cgv_db_cur.execute(v_sql);
                

    def cgf_update_job_etime(self,v_job_id = None,v_job_name = ""):
        """设置job的结束时间"""
        if (self.cgv_db_cur):
            v_sql = """update q_job_list set 
                    gmt_modify = CURRENT_TIMESTAMP,
    ...
    def cgf_update_job_endinfo(self,v_job_id = None,v_job_name = ""):
        """在job完成时，根据其子task的状态，更新最后的job信息，包括etime，status等"""
        if (self.cgv_db_cur):
            v_job_status = "";
            v_sql = """select a.id,count(*) as task_count,
                    sum(case b.task_status when 'finished' then 1 else 0 End) as finish_count,
                    sum(case b.task_status when 'error' then 1 else 0 End) as error_count,
                    min(b.begin_time) as begin_time,max(b.end_time) as end_time
                    from q_job_list a,q_task_list b
                    where a.id = b.job_id
                    and (a.id = %s or a.job_name = '%s')
                    group by a.id""" % (v_job_id,v_job_name);
            self.cgv_db_cur.execute(v_sql);
            v_rows_list = self.cgv_db_cur.fetchall();

            if (len(v_rows_list) > 0):
                if (v_rows_list[0][1] == v_rows_list[0][2]):
                    v_job_status = "finished";
                else:
                    v_job_status = "gooooing";

                if (v_rows_list[0][3] > 0):
                    v_job_status = "error"

            if (v_job_status == "finished"):
                v_sql = """update q_job_list 
                        set job_status = 'finished',
                        begin_time = '%s',end_time = '%s'
                        where id = %s""" % \
                        (str(v_rows_list[0][4]),str(v_rows_list[0][5]),v_job_id);
            elif (v_job_status == "gooooing"):
                v_sql = """update q_job_list 
                        set job_status = 'gooooing',
                        begin_time = '%s'
                        where id = %s""" % \
                        (str(v_rows_list[0][4]),v_job_id);
            elif (v_job_status == "error"):
                v_sql = """update q_job_list 
    ...
    def cgf_get_job_tasks(self,v_job_id = 0,v_job_name = "",v_task_type = ""):
        """获取一个job下的task列表,按照vtable／twins进行分类"""
        try:
            if (self.cgv_db_cur):
                v_job_rooters_list = [];
                v_rooter_tasks_list = {};
                ...
                # 获取一个job下面的vtable／twins，统称为rooter，最终由他们去展开自己的子任务
                # 获取vtable的task list，注意会有jobid的限制
                if (v_task_type == "pull"):
                    v_sql = """select distinct b.job_id,c.vtable_id as root_id,b.task_type
                            from q_job_list a,q_task_list b,q_table_sharding_list c
                            where a.id = b.job_id 
                            and b.task_type = '%s'
                            and b.parent_id = c.id
                            and (a.id = %s or a.job_name = '%s')""" % (v_task_type,v_job_id,v_job_name);

                # 获取twins下的task list，注意会有jobid的限制
                if (v_task_type == "twins"):
                    v_sql = """select distinct b.job_id,c.id as root_id,b.task_type
                            from q_job_list a,q_task_list b,q_tab_twins_list c
                            where a.id = b.job_id 
                            and b.task_type in ('check','recheck','report','repair')
                            and b.parent_id = c.id
                            and (a.id = %s or a.job_name = '%s')""" % (v_job_id,v_job_name);

                self.cgv_db_cur.execute(v_sql);
                v_job_rooters_list = self.cgv_db_cur.fetchall();

                # 由每个rooter，去生成自己的task列表
                for i in xrange(len(v_job_rooters_list)):
                    # 拖数据的任务，需要聚拢到vtable层面
                    if (v_job_rooters_list[i][2] == "pull"):
                        v_sql = """select b.id as task_id,b.task_name
                                from q_task_list b,q_table_sharding_list c
                                where b.task_type = 'pull'
                                and b.parent_id = c.id
                                and b.job_id = %s
                                and c.vtable_id = %s
                                """ % (v_job_id,v_job_rooters_list[i][1]);
                                self.cgv_db_cur.execute(v_sql);
                                v_task_list = self.cgv_db_cur.fetchall();
                                v_rooter_tasks_list[v_job_rooters_list[i][2] + str(v_job_rooters_list[i][1])] = v_task_list;

                    # check校验任务，基本是和twins一对一
                    if (v_job_rooters_list[i][2] in ("check","recheck","report","repair")):
                        v_sql = """select b.id as task_id,b.task_name
                                from q_task_list b,q_tab_twins_list c
                ...
                return v_job_rooters_list,v_rooter_tasks_list;
        except Exception as e:
            print "QMetaHooker.cgf_get_job_tasks get error:" + str(e).replace("'","");
            os._exit(0);
        
                

    def cgf_create_task_info(self,v_task_name = "",v_job_id = None,v_parent_id = None,v_task_type = None):
        """任务创建函数，返回taskid，失败的话程序直接退出"""
        try:
            v_task_id = 0;
            if (self.cgv_db_cur):
                if (len(v_task_name) < 1):
                    v_task_name = "Task:" + str(datetime.datetime.now()) + "." + self.cgf_get_unique_key();

                v_sql = "insert into q_task_list (task_name,task_status,task_type,parent_id,begin_time,job_id) " + \
                "values('%s','preparing','%s',%s,CURRENT_TIMESTAMP,%s)" % (v_task_name,v_task_type,v_parent_id,v_job_id);
    ...
    def cgf_update_task_hostpid(self,v_task_id = None,v_who_doing = ""):
        """设置task的主机名和进程号"""
        if (self.cgv_db_cur):
            v_sql = """update q_task_list set task_host_pid = '%s',
                    gmt_modify = CURRENT_TIMESTAMP
                    where id = %s""" % (v_who_doing,v_task_id);
            self.cgv_db_cur.execute(v_sql);
    ...
    def cgf_create_vtable_info(self,v_vtab_name = "",v_stab_name = "",v_vtab_status = "",v_source_target = "",v_vtable_ctype = "new"):
        """virtual table信息处理,v_vtable_ctype为new，append，recreate"""
        try:
            v_vtable_id = 0;
            v_stab_exname = "";
            if (self.cgv_db_cur):
                # 对于vtable，保持全局唯一，主要是便于任务间的共享,如存在直接返回vtable_id
                v_sql = """select id,single_table_name from q_virtual_table_list 
                where virtual_table_name = '%s'""" % \
                (v_vtab_name);
                self.cgv_db_cur.execute(v_sql);
                v_rows_list = self.cgv_db_cur.fetchall();
                ...
                if (len(v_rows_list) > 0):
                    # vtable已经存在
                    if (v_vtable_ctype == "new"):
                        # new表示新建vtable，由于已经存在，返回-1
                        v_vtable_id = -1;
                        v_stab_exname = "";
                    elif (v_vtable_ctype == "append"):
                        # append表示扩展目前的vtable，直接返回vtable id
                        v_vtable_id = v_rows_list[0][0];
                        v_stab_exname = v_rows_list[0][1];
                    elif (v_vtable_ctype == "recreate"):
                        v_vtable_id = v_rows_list[0][0];
                        v_stab_exname = v_rows_list[0][1];
                        # 重置vtable的状态到preparing
                        self.cgf_update_vtable_status(v_vtable_id = v_vtable_id,v_vtable_status = "preparing");
                        self.cgf_update_vtable_trows(v_vtable_id = v_vtable_id,v_table_rows = 0,v_type = "final");
                        self.cgv_conn_db.commit();

                    return v_vtable_id,v_stab_exname;

                else:
                    # vtable不存在
                    if (v_vtable_ctype == "new" or v_vtable_ctype == "recreate"):
                        # new和recreate直接新建
                        ...
                        return v_vtable_id,v_stab_name;
                    elif (v_vtable_ctype == "append"):
                        # append没有vtable，不能进行
                        return -2,v_stab_name;
        except Exception, e:
            print "QMetaHooker.cgv_create_vtable_info get error:" + str(e).replace("'","");
            self.cgv_conn_db.rollback();
            os._exit(0);

    def cgf_update_jobs_vtable(v_job_id = 0,v_vtable_id = 0):
        """更新vtable在某个具体job任务下的信息，包括status，各种时间等"""
        if (self.cgv_db_cur):
            v_vtable_status = "";
            v_sql = """select c.vtable_id,count(*) as task_count,
                    sum(case b.task_status when 'finished' then 1 else 0 End) as finish_count,
                    sum(case b.task_status when 'error' then 1 else 0 End) as error_count,
                    min(c.countoff_btime) as countoff_btime,max(c.countoff_etime) as countoff_etime,
                    min(c.pull_btime) as pull_btime,max(c.pull_etime) as pull_etime
                    from q_task_list b,q_table_sharding_list c
                    where b.job_id = %s
                    and b.task_type = 'pull'
                    and b.parent_id = c.id
                    and c.vtable_id = %s
                    group by c.vtable_id""" % (v_job_id,v_vtable_id);
            self.cgv_db_cur.execute(v_sql);
            v_rows_list = self.cgv_db_cur.fetchall();

            # task应该全部是finished状态，vtable才能是pulled，否则就是pulling，如果有error，会覆盖pulling，作为错误
            if (len(v_rows_list) > 0):
                if (v_rows_list[0][1] == v_rows_list[0][2]):
                    v_vtable_status = "pulled";
                else:
                    v_vtable_status = "pulling";

                if (v_rows_list[0][3] > 0):
                    v_vtable_status = "error"

            if (v_vtable_status == "pulled"):
                v_sql = """update q_virtual_table_list 
                        set virtual_table_status = 'pulled',
                        countoff_btime = '%s',countoff_etime = '%s',
                        pull_btime = '%s',pull_etime = '%s'
                        where id = %s""" % \
                        (str(v_rows_list[0][4]),v_rows_list[0][5],v_rows_list[0][6],v_rows_list[0][7],v_vtable_id);
            elif (v_vtable_status == "pulling"):
                v_sql = """update q_virtual_table_list 
                        set virtual_table_status = 'pulling',
                        countoff_btime = '%s',countoff_etime = '%s',
                        pull_btime = '%s'
                        where id = %s""" % \
                        (str(v_rows_list[0][4]),v_rows_list[0][5],v_rows_list[0][6],v_vtable_id);
            elif (v_vtable_status == "error"):
                v_sql = """update q_virtual_table_list 
                        set virtual_table_status = 'error'
    ...
    def cgf_get_vtable_stable(self,v_vtable_id = 0):
        """vtable id获取stable name"""
        try:
            v_stable_name = "";
            if (self.cgv_db_cur):
                v_sql = """select single_table_name 
                        from q_virtual_table_list where id = %s""" % (v_vtable_id);

                self.cgv_db_cur.execute(v_sql);
                v_rows_list = self.cgv_db_cur.fetchall();

                if (len(v_rows_list) > 0):
                    v_stable_name = v_rows[0][0];

            return v_stable_name;
        except Exception, e:
            print "QMetaHooker.cgf_get_vtable_stable get error:" + str(e).replace("'","");

    def cgf_reset_vtable_info(self,v_vtable_id = 0):
        """清理vtable的子table和task任务信息"""
        try:
            if (self.cgv_db_cur):
                self.cgv_db_cur.execute("""delete from q_task_list 
                    where task_type = 'pull'
                    and parent_id in 
                    (select id from q_table_sharding_list 
                    where vtable_id = %s)""" % (v_vtable_id));
                self.cgv_db_cur.execute("delete from q_table_sharding_list where vtable_id = %s" % (v_vtable_id));
                self.cgv_db_cur.execute("""update q_virtual_table_list 
                    set countoff_btime = Null,countoff_etime = Null,
                    pull_btime = Null,pull_etime = Null,
                    virtual_table_status = 'preparing',error_info = ''
                    where id = %s""" % (v_vtable_id));
                self.cgv_conn_db.commit();

        except Exception, e:
            self.cgv_conn_db.rollback();
            print "QMetaHooker.cgf_reset_vtable_info get error:" + str(e).replace("'","");
            os._exit(0);
    ...
    def cgf_create_tabshard_info(self,v_vtable_id = None,v_sharding_table_name = "",
        ...
        try:
            ...
            if (self.cgv_db_cur):
                # 在一个vtable里面，子表不要重复
                v_sql = """select id from q_table_sharding_list 
                where vtable_id = %s and sharding_table_name = '%s' 
                and sharding_table_owner = '%s' and ip = '%s' and port = %s 
                and sid_or_dbname = '%s' and db_type = '%s'""" % \
                (v_vtable_id,v_sharding_table_name,v_sharding_table_owner,
                v_ip,v_port,v_sid_or_dbname,v_db_type);
                self.cgv_db_cur.execute(v_sql);
                v_rows_list = self.cgv_db_cur.fetchall();
                if (len(v_rows_list) > 0):
                    # 存在，返回老的tableid
                    v_tabshard_id = v_rows_list[0][0];
    ...
    def cgf_create_twins_info(self,v_svtab_name = "",v_tvtab_name = "",v_svtab_id = None,v_tvtab_id = None,v_twins_status = ""):
        ...
        try:
            ...
            if (self.cgv_db_cur):
                ...
                # 已经存在该vtable的配对twins，返回老的twins信息
                v_sql = """select id,twins_name from q_tab_twins_list 
                where source_vtable_name = '%s' and target_vtable_name = '%s'""" % \
                (v_svtab_name,v_tvtab_name);
                self.cgv_db_cur.execute(v_sql);
                v_rows_list = self.cgv_db_cur.fetchall();
                if (len(v_rows_list) > 0):
                    return v_rows_list[0][0],v_rows_list[0][1]
                else:
                    # 不存在，直接生成
    ...
    def cgf_reset_twins_info(self,v_twins_id = 0):
        """清理twins的task任务，谨慎使用"""
        try:
            if (self.cgv_db_cur):
                self.cgv_db_cur.execute("""delete from q_task_list 
                    where task_type in ('check','report','repair','recheck')
                    and parent_id  = %s""" % (v_twins_id));
                self.cgv_conn_db.commit();

        except Exception, e:
            self.cgv_conn_db.rollback();
            print "QMetaHooker.cgf_reset_twins_info get error:" + str(e).replace("'","");
            os._exit(0);
    ...
    def cgf_get_twins_info(self,v_twins_id = None,v_twins_name = ""):       
        ...
        try:
            ...
            if (self.cgv_db_cur):
                ...
                where (a.id = %s or a.twins_name = '%s')
                and a.source_vtable_id = s.id
                and a.target_vtable_id = t.id""" % (v_twins_id,v_twins_name);
                self.cgv_db_cur.execute(v_sql);
    ...
    def cgf_get_tabsharding_info(self,v_sharding_id = None):
        """获取tabsharding相关的信息，用于tablepipe的初始化信息"""
        try:
            v_tabsharding_info = {};
            if (self.cgv_db_cur):
                v_sql = """select b.vtable_id,b.id as sharding_id,
                        b.sql_select,b.sql_where,b.pkuk_str,
                        b.sharding_str,b.ip,b.port,b.sid_or_dbname,b.db_user,
                        b.db_pwd,b.process_parallel,b.db_type,b.sharding_table_name,
                        b.sharding_table_owner,c.single_table_name
                        from q_table_sharding_list b,q_virtual_table_list c
                        where b.id = %s and b.vtable_id = c.id""" % (v_sharding_id);
                self.cgv_db_cur.execute(v_sql);
                v_tabsharding_col_desc = self.cgv_db_cur.description;
                v_rows_list = self.cgv_db_cur.fetchall();
                if (len(v_rows_list) > 0):
                    # 仅仅处理第一行数据，列名根据desc来进行，以col:value的模式放入v_tabsharding_info map
                    for i in xrange(len(v_tabsharding_col_desc)):
                        v_tabsharding_info[v_tabsharding_col_desc[i][0].lower()] = v_rows_list[0][i];

                return v_tabsharding_info;
        except Exception as e:
            print "QMetaHooker.cgf_get_tabsharding_info get error:" + str(e).replace("'","");

    def cgf_get_task_info(self,v_task_id = None,v_task_name = ""):
        """该函数用于获取一个task的状态，类型，及关联号parentid"""
        if (self.cgv_db_cur):
            v_task_type = "";
            v_task_status = "";
            v_parent_id = 0;
            ...
            v_task_spid = "";
            v_rows_list = [];
            v_sql = """select task_type,task_status,parent_id,task_host_pid from q_task_list 
                    where (id = %s or task_name = '%s')""" % (v_task_id,v_task_name);
            self.cgv_db_cur.execute(v_sql);
            v_rows_list = self.cgv_db_cur.fetchall();
            if (len(v_rows_list) > 0):
                v_task_type,v_task_status,v_parent_id,v_who_doing = v_rows_list[0][0],v_rows_list[0][1],v_rows_list[0][2],v_rows_list[0][3];

                # spid处理：993873@hostname
                if (v_who_doing is not None and len(v_who_doing) > 0):
                    v_task_spid = v_who_doing.split("@")[0];

            return v_task_type,v_task_status,v_parent_id,v_task_spid;

    def cgf_doner_doing_log(self,v_doner_name = "",v_doner_type = "",v_some_info = "",v_is_what = ""):
        """操作日志流水记录"""
        if (self.cgv_db_cur):
            v_sql = """insert into q_done_log(doner_name,doner_type,some_info)
            values('%s','%s','%s')""" % (v_doner_name,v_doner_type,v_some_info);
            self.cgv_db_cur.execute(v_sql);
                
class QTablePipe():
    """TablePipe，负责表数据的抽取，转换，加载,主要基于数据库来计算"""
    def __init__(self,v_fetch_MB = 256,v_is_need_data = True,
                v_is_json_data = True,v_tab_owner_target = None,v_who_doing = "",
                v_source_db = None,v_crusher_db = None,v_meta_hooker = None,
                v_hash_type = "hash",v_line_factor = None,v_process_parallel = 1,
                v_task_id = 0,v_sharding_id = 0,v_split_str = unichr(8364).encode("utf-8")):
        # 查询SQL信息
        self.cgv_sql = "";
        self.cgv_count_sql = "";
        ...
        self.cgv_key_sql = "";
        self.cgv_sql_where = "";
        self.cgv_select_cols = "";
        self.cgv_selectcol_list = "";
        # 列描述信息
        self.cgv_curcol_desc_list = None;
        # key信息
        self.cgv_pkuk_str = "";
        self.cgv_sharding_str = "";
        self.cgv_key_str = "";
        self.cgv_key_list = "";
        # 查询并行度，oracle有效
        self.cgv_process_parallel = v_process_parallel;
        # 源端表信息
        self.cgv_tab_owner_source = "";
        self.cgv_tab_name_source = "";
        # 目标端表信息
        self.cgv_tab_owner_target = v_tab_owner_target;
        self.cgv_tab_name_target = "";
        # 数据库信息
        self.cgv_crusher_db = v_crusher_db;
        ...
        self.cgv_split_str = v_split_str;

        # task的host pid
        self.cgv_who_doing = v_who_doing;

        ###################### 初始化信息 ######################
        # 初始化tabsharding信息
        self.cgf_init_tabsharding_info();
    ...
    def cgf_init_tabsharding_info(self):
        """初始化tabsharding相关的信息"""
        v_tabsharding_info = self.cgv_meta_hooker.cgf_get_tabsharding_info(v_sharding_id = self.cgv_sharding_id);

        self.cgv_sql_where = v_tabsharding_info.get("sql_where");
        self.cgv_select_cols = v_tabsharding_info.get("sql_select").lower().replace(" ","");
        self.cgv_selectcol_list = v_tabsharding_info.get("sql_select").lower().replace(" ","").split(",");
        # key信息
        self.cgv_pkuk_str = v_tabsharding_info.get("pkuk_str").lower().replace(" ","").rstrip(",").lstrip(",");
        self.cgv_sharding_str = v_tabsharding_info.get("sharding_str").lower().replace(" ","").rstrip(",").lstrip(",");
        self.cgv_key_str = self.cgv_pkuk_str + "," + self.cgv_sharding_str;
        self.cgv_key_list = self.cgv_key_str.split(",");
        # 查询并行度，oracle有效
        # self.cgv_process_parallel = v_tabsharding_info.get("process_parallel");
        # 源端表信息
        self.cgv_tab_owner_source = v_tabsharding_info.get("sharding_table_owner");
        self.cgv_tab_name_source = v_tabsharding_info.get("sharding_table_name");
        # 目标端表信息
        self.cgv_tab_name_target = v_tabsharding_info.get("single_table_name");
        # taskid,twinsid,shardingid初始化
        self.cgv_vtable_id = v_tabsharding_info.get("vtable_id"); 
    ...
    def cgf_get_key_cols(self):
        ...
        try:
            ...
            if (self.cgv_source_db.cgv_db_cur):
                ...
                if (len(self.cgv_pkuk_str) < 1):
                    ...
                    if (len(self.cgv_pkuk_str) < 1 or self.cgv_pkuk_str is None):
                        ...
                        self.cgv_meta_hooker.cgf_commit_trans();
                        os._exit(0);
                    else:
                        # 反填pkuk str
            ...
            self.cgv_meta_hooker.cgf_rollback_trans();
            self.cgv_meta_hooker.cgf_update_sharding_status(v_sharding_id = self.cgv_sharding_id,
    ...
    def cgf_estimate_blockrows(self,v_set_col_desc = True,v_count_off = True):
        ...
        try:
            ...
            if (v_count_off == True):
                ...
                if (self.cgv_source_db.cgv_db_cur):
                    # 记录进程信息
                    self.cgv_meta_hooker.cgf_update_task_hostpid(v_task_id = self.cgv_task_id,v_who_doing = self.cgv_who_doing);
                    # 该tabshard进入到统计状态
                    self.cgv_meta_hooker.cgf_update_sharding_status(v_sharding_id = self.cgv_sharding_id,
                    ...
                    self.cgv_table_rows = v_row_count;
    ...
    def cgf_get_keystr_value(self,v_row):
        ...
        for i in xrange(len(self.cgv_keycol_position)):
            # 分隔符为 €
            v_key_str = v_key_str + str(self.cgf_unicode_to_string(v_row[self.cgv_keycol_position[i]])) + self.cgv_split_str;

        return v_key_str.rstrip(self.cgv_split_str);
    ...
    def cgf_get_datastr_value(self,v_row = None):
        ...
        for i in xrange(len(self.cgv_datacol_position)):
            # 分隔符为 €
            v_data_str = v_data_str + str(self.cgf_unicode_to_string(v_row[self.cgv_datacol_position[i]])) + self.cgv_split_str;

        if (self.cgv_line_factor is None):
            return v_data_str.rstrip(self.cgv_split_str);
        else:
            # 换行填充因子存在，进行替换
            return v_data_str.rstrip(self.cgv_split_str).replace("\r",self.cgv_line_factor).replace("\n",self.cgv_line_factor);
        ...
        if (len(self.cgv_data_block) > 0):
            try:
                ...
                for i in xrange(len(self.cgv_data_block)):
                    ...
                    # 生成datastr
                    if (self.cgv_is_json_data == False):
                        v_data_str = self.cgf_get_datastr_value(self.cgv_data_block[i]);
                    else:
                        v_data_str = self.cgf_get_datastr_json(self.cgv_data_block[i]);
        ...
        try:
                ...
                if (self.cgv_source_db.cgv_db_cur):
                    ...
                    while (v_is_continue):
                        ...
                        if (len(self.cgv_data_block) > 0):
                            ...
                            v_pulled_percent = str(format(float(v_count_rows*100) / float(self.cgv_table_rows),".2f")) + "%";
                            self.cgv_meta_hooker.cgf_update_task_percent(v_task_id = self.cgv_task_id,
                                    v_pulled_percent = v_pulled_percent);
                            self.cgv_meta_hooker.cgf_commit_trans();
            ...
            # MySQL数据库的抽取转换
                ...
                if (self.cgv_source_db.cgv_db_sscur):                    
                    ...
                    self.cgv_meta_hooker.cgf_update_sharding_status(v_sharding_id = self.cgv_sharding_id,
                        v_sharding_status = "pulled",v_error_info = "");
                    self.cgv_meta_hooker.cgf_update_task_hostpid(v_task_id = self.cgv_task_id,v_who_doing = "");
                    self.cgv_meta_hooker.cgf_update_task_etime(v_task_id = self.cgv_task_id);
                    self.cgv_meta_hooker.cgf_update_task_status(v_task_id = self.cgv_task_id,
                        v_task_status = "finished",v_error_info = "");
    ...
    def cgf_home_working(self,v_who_doing = ""):
        """work函数，便于process的统一调度"""
        self.cgv_who_doing = v_who_doing;
        self.cgf_estimate_blockrows(v_set_col_desc = False,v_count_off = True);
        self.cgf_data_transfor();

class QTableCrusher():
    """负责数据的对比校验，支持数据block，数据表，对比完毕生成结果"""
    def __init__(self,v_task_id = 0,v_twins_id = 0,v_process_parallel = 1,
                v_crusher_db = None,v_meta_hooker = None,v_who_doing = "",
    ...
    def cgf_home_working(self,v_who_doing = ""):
        """work函数，便于process的统一调度"""
        self.cgv_who_doing = v_who_doing;
        self.cgf_twinstab_crusher();

class QFatWorker(multiprocessing.Process):
    """QFatWorker仅仅负责run，多进程run，每个进程自带run逻辑"""
    def __init__(self,v_what_is_fucking_goooon):
        multiprocessing.Process.__init__(self);
        self.cgv_what_is_fucking_goooon = v_what_is_fucking_goooon;

    def run(self):
        if (self.cgv_what_is_fucking_goooon):
            # 当前进程的pid@hostname信息，做为whodo塞给cgv_what_is_fucking_goooon
            v_who_doing = str(os.getpid()) + "@" + socket.gethostname();
            # 每个cgv_what_is_fucking_goooon都会实现home working函数，封装好自身的运行逻辑，任务状态更新自己处理
            self.cgv_what_is_fucking_goooon.cgf_home_working(v_who_doing = v_who_doing);
        else:
            print "FatWorker get error : cgv_what_is_fucking_goooon is null!!!"

class QWorkerBoss():
    """WorkerBoss类，负责任务分配，多进程任务调度与管理，整个体系联动的操盘手"""
    def __init__(self):
        # worker数量定义
        self.cgv_worker_max = None;
        # 进程轮询等待时间
        self.cgv_sleep_seconds = None;
        # 抽数据的SQL并发度
        self.cgv_pull_process_parallel = None;
        # 对比SQL的并发度
        self.cgv_check_process_parallel = None;
        # 拖数据进程内存大小限制
        self.cgv_pull_process_sizeMB = None;
        # 是否字面值对比
        self.cgv_literate_check = None;
        # 是否用json类型格式化data数据
        self.cgv_json_data = None;
        # 是否保留抽取的原始数据
        self.cgv_save_data = None;
        # 是否需要做反向多余数据校验
        self.cgv_need_surplus = None;
        # hash算法
        self.cgv_hash_type = None;
        # 换行符的填充因子
        self.cgv_line_factor = "x86";
        # datastr的连接分隔符号
        self.cgv_split_str = unichr(8364).encode("utf-8");


        # 活动worker数量
        self.cgv_working_count = 0;

        # worker进程池
        self.cgv_worker_buss = [];

        # 对比库，meta库信息，meta hooker
        self.cgv_crusherdb_info = {};
        self.cgv_metadb_info = {};
        self.cgv_meta_hooker = None;

    def cgf_init_boss_conf(self):
        """根据conf文件初始化boss配置"""
        try:
            v_conf_file = ConfigParser.ConfigParser();
            v_conf_file.read("q_data_checkX.conf");
            # 通用配置项
            self.cgv_sleep_seconds = v_conf_file.get("ConfSet",'sleep_seconds');
            self.cgv_pull_process_parallel = v_conf_file.get("ConfSet",'pull_process_parallel');
            self.cgv_check_process_parallel = v_conf_file.get("ConfSet",'check_process_parallel');
            self.cgv_pull_process_sizeMB = v_conf_file.get("ConfSet",'pull_process_sizeMB');
            self.cgv_literate_check = v_conf_file.get("ConfSet",'literate_check');
            self.cgv_json_data = v_conf_file.get("ConfSet",'json_data');
            self.cgv_save_data = v_conf_file.get("ConfSet",'save_data');
            self.cgv_hash_type = v_conf_file.get("ConfSet",'hash_type');
            self.cgv_worker_max = v_conf_file.get("ConfSet",'worker_process_count');
            self.cgv_need_surplus = v_conf_file.get("ConfSet",'need_surplus');

            # 对比库crusherdb
            self.cgv_crusherdb_info["db_ip"] = v_conf_file.get("CrusherDB",'db_id');
            self.cgv_crusherdb_info["db_port"] = v_conf_file.get("CrusherDB",'db_port');
            self.cgv_crusherdb_info["sid_or_dbname"] = v_conf_file.get("CrusherDB",'sid_or_dbname');
            self.cgv_crusherdb_info["db_user"] = v_conf_file.get("CrusherDB",'db_user');
            self.cgv_crusherdb_info["db_pwd"] = v_conf_file.get("CrusherDB",'db_pwd');
            self.cgv_crusherdb_info["db_type"] = v_conf_file.get("CrusherDB",'db_type');

            # meta元信息库
            self.cgv_metadb_info["db_ip"] = v_conf_file.get("MetaDB",'db_id');
            self.cgv_metadb_info["db_port"] = v_conf_file.get("MetaDB",'db_port');
            self.cgv_metadb_info["sid_or_dbname"] = v_conf_file.get("MetaDB",'sid_or_dbname');
            self.cgv_metadb_info["db_user"] = v_conf_file.get("MetaDB",'db_user');
            self.cgv_metadb_info["db_pwd"] = v_conf_file.get("MetaDB",'db_pwd');
            self.cgv_metadb_info["db_type"] = v_conf_file.get("MetaDB",'db_type');
            v_meta_db = QConnectDB(v_ip = self.cgv_metadb_info.get("db_ip"),
                v_port = self.cgv_metadb_info.get("db_port"),
                v_sid_or_dbname = self.cgv_metadb_info.get("sid_or_dbname"),
                v_db_user = self.cgv_metadb_info.get("db_user"),
                v_dbpwd = self.cgv_metadb_info.get("db_pwd"),
                v_dbtype = self.cgv_metadb_info.get("db_type"));
            self.cgv_meta_hooker = QMetaHooker(v_meta_db = v_meta_db);      
        except Exception,e:
            print "QWorkerBoss.cgf_init_boss_conf get error :" + str(e).replace("'","");
            os._exit(0);

    def cgf_get_aworker_count(self):
        """获取正在运行的worker数量"""
        try:
            v_worker_count = 0;
            for i in xrange(len(self.cgv_worker_buss)):
                if (self.cgv_worker_buss[i].is_alive() == True):
                    v_worker_count = v_worker_count + 1;

            return v_worker_count;
        except Exception as e:
            print "QWorkerBoss.cgf_get_aworker_count get error :" + str(e).replace("'","");
            return 0;
        
    def cgf_kill_task(self,v_task_spid = None):
        """task任务进程kill"""
        try:
            if (v_task_spid > 0):
                v_kill_handle = os.kill(v_task_spid, signal.SIGKILL)
                print 'Kill task process :%s,return value is :' % (v_task_spid,v_kill_handle);
        except OSError, e:
            print "QWorkerBoss.cgf_kill_task get error :" + str(e).replace("'","");

    def cgf_tablecat_jobtasks(self,v_job_name = "",v_job_ctype = "new"):
        """任务元信息生成，tablecat的用武之地，v_job_ctype:new/append/recreate"""
        pass;

    def cgf_doing_myjob(self,v_job_id = None,v_job_name = "",v_run_type = "noforce"):
        """根据jobid或jobname来工作，runtype为force：强制执行，noforce: 根据规则判断执行"""
        # 一个job下的task，先运行pull，保证数据全部拖过来，pull完成后，才能开始check，recheck，repair，report等任务

        # 开始pull的task，即vtable下面的task
        v_job_vtable_list,v_vtable_tasks_list = self.cgv_meta_hooker.cgf_get_job_tasks(v_job_id = v_job_id,v_job_name = v_job_name,v_task_type = "pull");

        # 更新job状态
        self.cgv_meta_hooker.cgf_update_job_status(v_job_id = v_job_id,v_job_name = v_job_name,v_job_status = "gooooing",v_error_info = "");
        self.cgv_meta_hooker.cgf_update_job_btime(v_job_id = v_job_id,v_job_name = v_job_name);
        self.cgv_meta_hooker.cgf_commit_trans();

        for i in xrange(len(v_job_vtable_list)):
            # 对于vtable的single table先做处理，noforce维持不动，force需要truncate
            if (v_run_type == "force"):
                self.cgv_meta_hooker.cgf_single_table_ddl(v_vtable_id = v_job_vtable_list[i][1],v_is_truncate = True);
            elif (v_run_type == "noforce")
                pass;

            # 获取当前rooter的task list
            v_task_list = v_vtable_tasks_list.get(v_job_vtable_list[i][2] + str(v_job_vtable_list[i][1]));

            for i in xrange(len(v_task_list)):
                # 分配任务运行
                self.cgf_doing_mytask(v_task_id = v_task_list[i][0],v_task_name = v_task_list[i][1],
                    v_job_id = v_task_list[i][2],v_run_type = v_run_type);

                # 获取当前活动worker数量
                self.cgv_working_count = self.cgf_get_aworker_count();

                # 当活动worker已经到达上限，按照等待周前进行轮训，直到有worker释放
                while (self.cgv_working_count >= self.cgv_worker_max):
                    # 等待self.cgv_sleep_seconds
                    time.sleep(self.cgv_sleep_seconds);
                    # 查看当前活动worker数量
                    self.cgv_working_count = self.cgf_get_aworker_count();

        # 收割所有的vtable的task任务
        for i in xrange(len(self.cgv_worker_buss)):
            self.cgv_worker_buss[i].join();

        # 开始更新vtable的状态
        for i in xrange(len(v_job_vtable_list)):
            self.cgv_meta_hooker.cgf_update_jobs_vtable(v_job_id = v_job_vtable_list[i][0],
                v_vtable_id = v_job_vtable_list[i][1]);
            self.cgv_meta_hooker.cgf_commit_trans();

        # 开始twins下的task
        v_job_twins_list,v_twins_tasks_list = self.cgv_meta_hooker.cgf_get_job_tasks(v_job_id = v_job_id,v_job_name = v_job_name,v_task_type = "twins");
        for i in xrange(len(v_job_twins_list)):  
            # 获取当前twins的task list
            v_task_list = v_twins_tasks_list.get(v_job_twins_list[i][2] + str(v_job_twins_list[i][1]));
        ...
        # 开始更新twins的状态，由于twins是单任务，不存在汇总处理，已经在task内部完成，此处不需要了
        pass;

        # 更新job状态,需要根据子task的状态来判断
        self.cgv_meta_hooker.cgf_update_job_endinfo(v_job_id = v_job_id,v_job_name = v_job_name);
        self.cgv_meta_hooker.cgf_commit_trans();

    def cgf_doing_mytask(self,v_task_id = None,v_task_name = "",v_job_id = 0,v_run_type = "noforce"):
        """根据taskid或taskname来工作，runtype为force：强制执行，noforce: 根据规则判断执行"""
        v_task_type,v_task_status,v_parent_id,v_task_spid = self.cgv_meta_hooker.cgf_get_task_info(v_task_id = v_task_id,v_task_name = v_task_name);
        if (v_task_type is None or len(v_task_type) <=0):
            # task信息丢失，直接退出
            return 0;

        ##################################### noforce模式 #####################################
        # 处于preparing,error的任务会进行处理，处于gooooing，finished的任务不会处理
        if (v_run_type = "noforce"):
            if (v_task_status in ("preparing","error")):
                # 出错或带运行的任务开始处理，preparing是不安全的状态，需要注意
                if (v_task_type == "pull"):
                    # 获取tabsharding信息
                    v_tabsharding_info = self.cgv_meta_hooker.cgf_get_tabsharding_info(v_sharding_id = v_parent_id);

                    # 初始化tabsharding的数据库连接
                    v_tabsharding_db = QConnectDB(v_ip = v_tabsharding_info.get("ip"),
                        v_port = v_tabsharding_info.get("port"),
                        v_sid_or_dbname = v_tabsharding_info.get("sid_or_dbname"),
                        v_db_user = v_tabsharding_info.get("db_user"),
                        v_dbpwd = v_tabsharding_info.get("db_pwd"),
                        v_dbtype = v_tabsharding_info.get("db_type"));
                    ...
                    # 初始化crusher db的数据库连接
                    v_crusher_db = QConnectDB(v_ip = self.cgv_crusherdb_info.get("db_ip"),
                    v_port = self.cgv_crusherdb_info.get("db_port"),
                    v_sid_or_dbname = self.cgv_crusherdb_info.get("sid_or_dbname"),
                    v_db_user = self.cgv_crusherdb_info.get("db_user"),
                    v_dbpwd = self.cgv_crusherdb_info.get("db_pwd"),
                    v_dbtype = self.cgv_crusherdb_info.get("db_type"));

                    # 初始化table pipe
                    v_my_tablepipe = QTablePipe(v_fetch_MB = self.cgv_pull_process_sizeMB,
                    v_is_need_data = self.cgv_save_data,v_is_json_data = self.cgv_json_data,
                    v_tab_owner_target = self.cgv_crusherdb_info.get("db_user"),
                    v_source_db = v_tabsharding_db,v_crusher_db = v_crusher_db,
                    v_meta_hooker = v_meta_hooker,v_hash_type = self.cgv_hash_type,
                    v_line_factor = self.cgv_line_factor,v_task_id = v_task_id,
                    v_sharding_id = v_parent_id,v_split_str = self.cgv_split_str,
                    v_process_parallel = self.cgv_pull_process_parallel);

                    # worker process生成
                    v_task_process = QFatWorker(v_what_is_fucking_goooon = v_my_tablepipe);
                    v_task_process.start();
                    # worker进入到buss
                    self.cgv_worker_buss.append(v_task_process);

                elif (v_task_type == "check"):
                    # 初始化meta db的数据库连接
                    v_meta_db = QConnectDB(v_ip = self.cgv_metadb_info.get("db_ip"),
                    v_port = self.cgv_metadb_info.get("db_port"),
                    v_sid_or_dbname = self.cgv_metadb_info.get("sid_or_dbname"),
                    ...
                    # 生成crusher粉碎机
                    v_my_crusher = QTableCrusher(v_task_id = v_task_id,v_twins_id = v_parent_id,
                    v_process_parallel = self.cgv_check_process_parallel,v_crusher_db = v_crusher_db,
                    v_meta_hooker = v_meta_hooker,v_need_surplus = self.cgv_need_surplus,
                    v_literate_check = self.cgv_literate_check);

                    # worker process生成
                    v_task_process = QFatWorker(v_what_is_fucking_goooon = v_my_crusher);
                    v_task_process.start();

                    # worker进入到buss
                    self.cgv_worker_buss.append(v_task_process);
                    
                elif (v_task_type == "report"):
                    # 报告生成任务
                    pass;
                    
                elif (v_task_type == "recheck"):
                    # recheck任务
                    pass;
                    
                elif (v_task_type == "repair"):
                    # 修复任务
                    pass;
                    
            elif (v_task_status in ("gooooing","finished")):
                # 正在进行中的,已经finished不做处理
                pass;

        ##################################### force模式 #####################################
        # 全部重来
        elif (v_run_type = "force"):
            # 杀掉任务的会话
            self.cgf_kill_task(v_task_spid = v_task_spid);
            # 开始处理
            if (v_task_type == "pull"):
                # 获取tabsharding信息
                v_tabsharding_info = self.cgv_meta_hooker.cgf_get_tabsharding_info(v_sharding_id = v_parent_id);
    ...
    def cgf_doing_myvtable(self,v_vtable_id = None,v_vtable_name = "",v_run_type = "noforce"):
        """根据vtableid或vtablename来工作，runtype为force：强制执行，noforce: 根据规则判断执行"""
        pass;

    def cgf_doing_mytwnis_check(self,v_twins_id = None,v_twins_name = "",v_run_type = "noforce"):
        """根据twinsid或twinsname来进行对比，runtype为force：强制执行，noforce: 根据规则判断执行"""
        pass;

    def cgf_doing_mytwnis_recheck(self,v_twins_id = None,v_twins_name = "",v_run_type = "noforce"):
        """根据twinsid或twinsname来进行重复对比，runtype为force：强制执行，noforce: 根据规则判断执行"""
        pass;

    def cgf_doing_mytwnis_repair(self,v_twins_id = None,v_twins_name = "",v_run_type = "noforce"):
        """根据twinsid或twinsname来进行数据修复，runtype为force：强制执行，noforce: 根据规则判断执行"""
        pass;
...
if __name__ == "__main__":
    ...
    # 拖数据task，生成vtable，调度层面应该考虑vtable的truncate
    v_sdb_pipe1 = QTablePipe(v_pkuk_str = "",v_sharding_str = "",v_fetch_MB = 256,
                v_sql_where = " limit 10000 ",v_select_cols = "",v_is_json_data = True,
                v_tab_owner_source = "qbench",v_tab_name_source = "my_a",v_is_need_data = True,
                ...
                v_process_parallel = 1,v_hash_type = "hash",v_task_id = 20,v_line_factor = "x86",
