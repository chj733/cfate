...
class QTableCat():
    ...
    def cgf_table_meta_gen(self):
        """1.vtable由多个或一个table组成，vtable一种是新建new，一种是append加子表，一种是recreate重建
           ...
           4.针对每个子table，会生成一个task，一直存在，这个task可供后续的各种任务调用
    ...
    def cgf_twins_meta_gen(self):
        ...
        try:
            for i in xrange(len(self.cgv_twins_data_list)):
                ...
                if (v_twins_id == -1):
                    print "Twins " + v_svtab_name + "-" + v_tvtab_name + " already existed,just ingore it!!!";
                else:
                    # 生成twins的任务,在twins gen里面，就是check,滞后到任务运行在启动
                    # self.cgv_meta_hooker.cgf_create_task_info(v_task_name = v_task_name,v_parent_id = v_twins_id,v_job_id = self.cgv_job_id,v_task_type = "check");
                    pass;
...
class QMetaHooker():
    ...
    def cgf_update_task_hostpid(self,v_task_id = None,v_host_pid = ""):
        """设置task的主机名和进程号"""
        if (self.cgv_db_cur):
            v_sql = """update q_task_list set task_host_pid = '%s',
                    gmt_modify = CURRENT_TIMESTAMP
                    where id = %s""" % (v_host_pid,v_task_id);
            self.cgv_db_cur.execute(v_sql);
    ...
    def cgf_create_tabshard_info(self,v_vtable_id = None,v_sharding_table_name = "",
        ...
        try:
            ...
            if (self.cgv_db_cur):
                # 在一个vtable里面，子表不要重复
                v_sql = """select count(*) as r_count from q_table_sharding_list 
...
class QTablePipe():
    ...
    def __init__(self,v_pkuk_str = "",v_sharding_str = "",v_fetch_MB = 256,
                ...
                v_tab_owner_target = None,v_tab_name_target = None,v_host_pid = "",
    ...
    def cgf_estimate_blockrows(self,v_set_col_desc = True,v_count_off = True):
        ...
        try:
            ...
            self.cgv_meta_hooker.cgf_update_task_hostpid(v_task_id = self.cgv_task_id,v_host_pid = self.cgv_host_pid);
            self.cgv_meta_hooker.cgf_commit_trans();
        ...
        try:
            ...
            # MySQL数据库的抽取转换
                ...
                if (self.cgv_source_db.cgv_db_sscur):                    
                    ...
                    self.cgv_meta_hooker.cgf_update_sharding_status(v_sharding_id = self.cgv_sharding_id,
                        v_sharding_status = "pulled",v_error_info = "");
                    self.cgv_meta_hooker.cgf_update_task_hostpid(v_task_id = self.cgv_task_id,v_host_pid = "");
                    self.cgv_meta_hooker.cgf_update_task_etime(v_task_id = self.cgv_task_id);
                    self.cgv_meta_hooker.cgf_update_task_status(v_task_id = self.cgv_task_id,
                        v_task_status = "finished",v_error_info = "");
...
class QTableCrusher():
    ...
    def __init__(self,v_task_id = 0,v_twins_id = 0,v_process_parallel = 1,
                v_crusher_db = None,v_meta_hooker = None,v_host_pid = "",
...
class QWorkerBoss():
    ...
    def __init__(self, arg):
        super(QWorkerBoss, self).__init__()
        self.arg = arg
...
class QFatWorker(multiprocessing.Process):
    ...
    def run(self):
        if (self.cgv_what_is_fucking_goooon):
            self.cgv_what_is_fucking_goooon.cgf_home_working();
        else:
