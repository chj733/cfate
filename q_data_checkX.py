...
class QTableCat():
    ...
    def __init__(self,v_meta_hooker = None,v_crusher_db = None,v_job_name = "",v_vtable_ctype = "new"):
        ...
        self.cgv_vtable_ctype = v_vtable_ctype;
...
class QMetaHooker():
    ...
    def cgf_create_job_info(self,v_job_name = ""):
        ...
        try:
            ...
            if (self.cgv_db_cur):
                # jobname重复，程序退出
                v_sql = "select count(*) as r_count from q_job_list where job_name = '%s'" % (v_job_name);
                self.cgv_db_cur.execute(v_sql);
                v_row_count = self.cgv_db_cur.fetchall()[0][0];
                if (v_row_count > 0):
                    print "Job " + v_job_name + " already existed,please change a different jobname!!!";
                    os._exit(0);
...
class QWorkerBoss():
    ...
    def cgf_doing_myjob(self,v_job_id = None,v_job_name = "",v_run_type = "noforce"):
        ...
        # 更新job状态
        self.cgv_meta_hooker.cgf_update_job_status(v_job_id = v_job_id,v_job_name = v_job_name,v_job_status = "finished",v_error_info = "");
        self.cgv_meta_hooker.cgf_update_job_etime(v_job_id = v_job_id,v_job_name = v_job_name);
        self.cgv_meta_hooker.cgf_commit_trans();
