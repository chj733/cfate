...
class QTableCat():
    ...
    def __init__(self,v_meta_hooker = None,v_crusher_db = None,v_job_name = "",v_vtable_ctype = "new"):
        ...
        self.cgv_job_id = 0;
        #对于vtable的创建采取merge合并还是new方式，用户可以利用merge扩展当前vtable，new方式是新建，存在就忽略
    ...
    def cgf_table_meta_gen(self):
        """1.vtable由多个或一个table组成，vtable一种是新建new，一种是已经存在做合并扩展
           2.一个vtable下的table不允许重复，程序会自动忽略重复table
           3.new vtable，如果已经存在，会忽略该vtable，merge vtable，如果存在，就直接扩展，不存在就直接新建并扩展
        """
        try:
            ...
            for i in xrange(len(self.cgv_virtual_tabname_scope)):
                ...
                if (v_vtable_id == -1):
                    print "Virtual table " + v_tab_vname + "(" + v_tab_sname + ")" + " already existed,just ignore!!!";
                    # 进入下个vtable的处理
                    continue;

                # 创建single table表,存放hash，keystr
    ...
    def cgf_twins_meta_gen(self):
        """生成校验对的meta信息
           按照源端vtable，目标端vtable进行配对，
           已经存在，直接利用现有twins的关系，不存在就新建
        """
        try:
...
class QMetaHooker():
    ...
    def cgf_create_vtable_info(self,v_vtab_name = "",v_stab_name = "",v_vtab_status = "",v_source_target = "",v_vtable_ctype = "new"):
        """创建virtual table信息,v_vtable_ctype表示新增vtable还是合并vtable,new|merge"""
        try:
            v_vtable_id = 0;
            if (self.cgv_db_cur):
                # 对于vtable，保持全局唯一，主要是便于任务间的共享,如存在直接返回vtable_id
                v_sql = """select id from q_virtual_table_list 
                where virtual_table_name = '%s' or single_table_name = '%s'""" % \
                (v_vtab_name,v_stab_name);
                self.cgv_db_cur.execute(v_sql);
                ...
                if (len(v_rows_list) > 0):
                    ...
                    if (v_vtable_ctype == "new"):
                        ...
                        v_vtable_id = -1;
                    elif (v_vtable_ctype == "merge"):
                        # merge表示扩展目前的vtable，直接返回vtable id
